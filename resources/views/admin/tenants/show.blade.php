<x-layouts.admin>
    ...
    <div class="max-w-5xl space-y-8">
        ...
        <a href="{{ route('admin.tenants.index') }}"
            class="inline-flex items-center text-xs font-bold text-slate-400 hover:text-[#2D7DD2] uppercase tracking-widest transition-colors group">
            <svg class="h-3 w-3 mr-2 transform group-hover:-translate-x-1 transition-transform" fill="none"
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-8 flex items-center justify-between border-b border-slate-100 bg-[#0F172A] text-white">
                <div class="flex items-center">
                    <div
                        class="h-16 w-16 rounded-lg bg-[#1E293B] border border-slate-700 flex items-center justify-center text-xl font-bold text-[#2D7DD2]">
            ...
            <!-- Metrics Grid -->
            <div
                class="grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-100 bg-slate-50/30">
                <div class="p-6 text-center">
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Active Workers</p>
                    <p class="mt-2 text-2xl font-black text-[#0F172A]">{{ number_format($metrics['workers_count']) }}
                    </p>
                </div>
                <div class="p-6 text-center">
                ...
                <div class="p-6 text-center">
                    ...
                    <p class="mt-2 text-2xl font-black text-emerald-600">
                        ₱{{ number_format($metrics['total_payroll_value'], 2) }}</p>
                </div>
            </div>
            ...
            <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-12 border-t border-slate-100">
                <div>
                    <h3 class="text-xs font-bold text-[#0F172A] uppercase tracking-widest mb-6">Administrative Contact
                    </h3>
                    <dl class="space-y-4">
                        <div>
                            <dt class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Primary
                                Administrator</dt>
                            <dd class="mt-1 text-sm font-bold text-[#0F172A]">{{ $tenant->admin_name }}</dd>
                        </div>
                        <div>
                            <dt class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Registered Email
                            </dt>
                            <dd class="mt-1 text-sm font-bold text-[#2D7DD2]">{{ $tenant->admin_email }}</dd>
                        </div>
                        <div>
                            <dt class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Database Identity
                            </dt>
                            <dd
                                class="mt-1 text-xs font-mono text-slate-500 bg-slate-100 px-2 py-1 rounded inline-block">
                                jobflow_{{ $tenant->id }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div>
                    ...
                    <dl class="space-y-4">
                        <div>
                            <dt class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Registration Date
                            </dt>
                            <dd class="mt-1 text-sm font-bold text-[#0F172A]">
                                {{ $tenant->created_at->format('F d, Y • h:i A') }}</dd>
                        </div>
                        <div>
                            <dt class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">System Lifetime
                            </dt>
                            <dd class="mt-1 text-sm font-bold text-[#0F172A]">{{ $tenant->created_at->diffForHumans() }}
                            </dd>
                        </div>
                        <div>
                            <dt class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Payroll Periods
                            </dt>
                            <dd class="mt-1 text-sm font-bold text-[#0F172A]">{{ $metrics['payroll_periods_count'] }}
                                cycles processed</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div class="p-6">
                    <form action="{{ route('admin.tenants.update', $tenant) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label for="brand_color"
                                    class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Primary
                                    Accent Color</label>
                                <div class="flex items-center space-x-3">
                                    <input type="color" name="brand_color" id="brand_color"
                                        value="{{ old('brand_color', $tenant->brand_color) }}"
                                        class="h-10 w-10 border-none rounded cursor-pointer">
                                    <input type="text" value="{{ $tenant->brand_color }}" readonly
                                        class="flex-1 bg-slate-50 border-slate-200 rounded text-xs font-mono text-slate-500">
                                </div>
                            </div>
                            <div>
                                <label for="logo_url"
                                    class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Logo
                                    URL (External)</label>
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                class="text-[10px] font-bold bg-[#0F172A] text-white px-4 py-2 rounded uppercase tracking-widest hover:bg-slate-800 transition-all">
                                Save Branding
                            </button>
                        </div>
                    </form>
        ...
        <div class="bg-slate-100 rounded-xl p-6 border border-slate-200">
            <div class="flex items-center space-x-3">
                ...
                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Isolated Infrastructure is
                    healthy and performing within nominal parameters.</p>
            </div>
        </div>
    </div>
</x-layouts.admin>