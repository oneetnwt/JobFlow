...
<body class="h-full industrial-theme bg-[var(--color-bg)] text-[var(--color-text-primary)]">
    ...
    <header class="app-header flex items-center justify-between px-[var(--space-6)]">
        <div class="flex items-center gap-[var(--space-3)] text-white w-[var(--sidebar-width)]">
            ...
            <span class="font-display text-[var(--text-xl)] text-foreground tracking-tight">Platform <span class="text-[var(--color-accent)]">Admin</span></span>
        </div>
    </header>
    ...
    <aside class="app-sidebar p-[var(--space-4)] flex flex-col justify-between">
        <nav class="flex flex-col gap-[var(--space-1)]">
            ...
            <a href="{{ route('admin.logs.index') }}" class="sidebar-nav-item {{ request()->routeIs('admin.logs.index') ? 'active' : '' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-6m4 6V7m4 10v-4M5 21h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                Audit Trail
            </a>
        </nav>
