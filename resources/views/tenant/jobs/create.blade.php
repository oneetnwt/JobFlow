            ...
            <div class="bg-white shadow-sm rounded-lg border border-slate-200 overflow-hidden">
                <div class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label for="deadline_at" class="block text-sm font-medium text-[#1E3A5F]">Deadline
                                (Optional)</label>
                    ...
                    <div>
                        <label for="assigned_to" class="block text-sm font-medium text-[#1E3A5F]">Assign to Worker
                            (Optional)</label>
                        <select id="assigned_to" name="assigned_to"
                            ...
                            @foreach($workers as $worker)
                                <option value="{{ $worker->id }}" {{ old('assigned_to') == $worker->id ? 'selected' : '' }}>
                                    {{ $worker->name }} ({{ $worker->roles->pluck('name')->implode(', ') }})
                                </option>
                            @endforeach
                        </select>
