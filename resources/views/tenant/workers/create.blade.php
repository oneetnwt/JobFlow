<x-layouts.tenant>
    ...
    <div class="max-w-4xl">
        <form action="{{ route('tenant.workers.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="bg-white shadow-sm rounded-lg border border-slate-200 overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-1">
                        <label for="password" class="block text-sm font-medium text-[#1E3A5F]">Initial Password</label>
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <select id="role" name="role"
                            ...
                            <option value="worker" {{ old('role') === 'worker' ? 'selected' : '' }}>Worker (Limited Access)</option>
                            <option value="manager" {{ old('role') === 'manager' ? 'selected' : '' }}>Manager (Team Management)</option>
                        </select>
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="date" name="joined_at" id="joined_at" value="{{ old('joined_at', date('Y-m-d')) }}"
                                   class="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#2D7DD2] focus:border-[#2D7DD2] sm:text-sm">
                    ...
                    <div>
                        ...
                        <textarea id="skills" name="skills" rows="3"
                                  class="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#2D7DD2] focus:border-[#2D7DD2] sm:text-sm"
                                  placeholder="e.g. Electrical License, Safety Certification, Welding...">{{ old('skills') }}</textarea>
                        @error('skills') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="px-6 py-4 bg-slate-50 text-right space-x-3">
                    <a href="{{ route('tenant.workers.index') }}" class="text-sm font-medium text-slate-600 hover:text-slate-800 transition-colors">Cancel</a>
                    <button type="submit"
