        ...
        <form action="{{ route('tenant.workers.update', $worker) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white shadow-sm rounded-lg border border-slate-200 overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-1">
                        <label for="password" class="block text-sm font-medium text-[#1E3A5F]">Update Password (Optional)</label>
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <select id="role" name="role"
                            ...
                            <option value="worker" {{ old('role', $worker->role) === 'worker' ? 'selected' : '' }}>Worker (Limited Access)</option>
                            <option value="manager" {{ old('role', $worker->role) === 'manager' ? 'selected' : '' }}>Manager (Team Management)</option>
                        </select>
            ...
            <div class="bg-white shadow-sm rounded-lg border border-slate-200 overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            <label for="employee_id" class="block text-sm font-medium text-[#1E3A5F]">Employee ID</label>
                            <input type="text" name="employee_id" id="employee_id" value="{{ old('employee_id', $worker->profile?->employee_id) }}"
                                   class="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#2D7DD2] focus:border-[#2D7DD2] sm:text-sm">
                        ...
                        <div>
                            ...
                            <input type="text" name="department" id="department" value="{{ old('department', $worker->profile?->department) }}"
                                   class="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#2D7DD2] focus:border-[#2D7DD2] sm:text-sm">
                            @error('department') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label for="employment_type" class="block text-sm font-medium text-[#1E3A5F]">Employment Type</label>
                            <select id="employment_type" name="employment_type"
                                    class="mt-1 block w-full bg-white border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#2D7DD2] focus:border-[#2D7DD2] sm:text-sm">
                                <option value="full-time" {{ old('employment_type', $worker->profile?->employment_type) === 'full-time' ? 'selected' : '' }}>Full-time</option>
                                <option value="part-time" {{ old('employment_type', $worker->profile?->employment_type) === 'part-time' ? 'selected' : '' }}>Part-time</option>
                                <option value="contract" {{ old('employment_type', $worker->profile?->employment_type) === 'contract' ? 'selected' : '' }}>Contract</option>
                                <option value="seasonal" {{ old('employment_type', $worker->profile?->employment_type) === 'seasonal' ? 'selected' : '' }}>Seasonal</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="hourly_rate" id="hourly_rate" value="{{ old('hourly_rate', $worker->profile?->hourly_rate ?? '0.00') }}"
                                   class="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#2D7DD2] focus:border-[#2D7DD2] sm:text-sm">
                            @error('hourly_rate') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                        
                        <div>
                            <label for="phone_number" class="block text-sm font-medium text-[#1E3A5F]">Phone Number</label>
                            <input type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $worker->profile?->phone_number) }}"
                                   class="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#2D7DD2] focus:border-[#2D7DD2] sm:text-sm">
                        ...
                        <div>
                            ...
                            <input type="date" name="joined_at" id="joined_at" value="{{ old('joined_at', $worker->profile?->joined_at?->format('Y-m-d')) }}"
                                   class="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#2D7DD2] focus:border-[#2D7DD2] sm:text-sm">
                    ...
                    <div>
                        <label for="skills" class="block text-sm font-medium text-[#1E3A5F]">Skills & Certifications</label>
                ...
                <div class="px-6 py-4 bg-slate-50 text-right space-x-3">
                    <a href="{{ route('tenant.workers.show', $worker) }}" class="text-sm font-medium text-slate-600 hover:text-slate-800 transition-colors">Cancel</a>
                    <button type="submit"
